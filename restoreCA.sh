...
# TLDR: This is a script to restore a Conversational agent from a GCS bucket
...
#   "https://us-central1-dialogflow.googleapis.com/v3/projects/uhgaa-434720/locations/us-central1/agents/902b3d95-7af8-4bcc-8799-a0dff07e7b04:restore" \
#   -d '{
...
AGENT_ID="902b3d95-7af8-4bcc-8799-a0dff07e7b04"
...
usage() {
  ...
  echo "  -b, --bucket-name  GCS Bucket Name for restore (default: ${BUCKET_NAME})"
  echo "  -o, --object-name  GCS Object Name of the agent to restore (default: ${OBJECT_NAME})"
...
# API URL for the restore operation
API_URL="${BASE_API_URL}/v3/projects/${PROJECT_ID}/locations/${LOCATION}/agents/${AGENT_ID}:restore"
...
echo "From Bucket: gs://${BUCKET_NAME}/${OBJECT_NAME}"
...
if [ -z "${OPERATION_NAME}" ] || [ "${OPERATION_NAME}" == "null" ]; then
  echo "Failed to initiate restore operation. Response:"
  echo "${OPERATION_RESPONSE}" | jq .
  exit 1
fi

echo -e "\nRestore operation initiated. Name: ${OPERATION_NAME}"
